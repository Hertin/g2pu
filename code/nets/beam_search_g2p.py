...
class BeamSearchG2P(torch.nn.Module):
    ...
    def init_hyp(self, x: torch.Tensor, xp: torch.Tensor) -> List[Hypothesis]:
    ...
    def search(
        self, running_hyps: List[Hypothesis], x: torch.Tensor, xp: torch.Tensor
    ) -> List[Hypothesis]:
        ...
        for hyp in running_hyps:
            ...
            part_scores, part_states = self.score_partial(hyp, part_ids, xp)
    ...
    def forward(
        self, x: torch.Tensor, xp: torch.Tensor, maxlenratio: float = 0.0, minlenratio: float = 0.0
    ) -> List[Hypothesis]:
        """Perform beam search.
        ...
        for i in range(maxlen):
            ...
            best = self.search(running_hyps, x, xp)
            # post process of one iteration
            running_hyps = self.post_process(i, maxlen, maxlenratio, best, ended_hyps)
            # logging.info(f'running_hyps {i} yseq {running_hyps.yseq}')
            # logging.info(f'running_hyps {i} scores {running_hyps.scores}')
